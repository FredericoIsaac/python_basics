...
class Jogo:
    ...
    def vencedor(self):
        ...
        resultado = False
        for trio in trios:
            resultado = reduce(and_, [self.obtem_estado(*elem) == jogador for elem in trio])
        if resultado:
            return True
        return False
...
class Ecran(Frame):
    ...
    def jogar(self, evento):
        self._erro = False
        mensagem = "Posicao ja ocupada..."
        posx = evento.x
        posy = evento.y
        x,y = self.quadrante(posx, posy)

        # Determina quadrante e centra caso esteja livre
        if self._jogo.livre(x,y):
            self._jogo.define_estado(x,y)
            self._desenha(x*150+75, y*150+75)
        else:
            showerror("Oops!", mensagem)
            self._erro = True
        if not self._erro:
            # Vencedor?
            if self._jogo.vencedor():
                showinfo("Uau!", "Ganhou %s" % self._jogo.obtem_jogador())
            else:
                # Muda de jogador
                self._jogo.muda_jogador()
                # Informa jogador
                self.jogador.set("Joga: " + self._jogo.obtem_jogador())
        else:
            self._erro = False

    def _desenha(self, x, y):
        if isinstance(self._jogo.obtem_tipo_jogador(), JogadorO):
            self.quadro1.create_oval(x-50, y-50, x+50, y+50, outline= "blue", width=3)
        else:
            self.quadro1.create_line(x-50, y-50, x+50, y+50, outline= "green", width=3)
            self.quadro1.create_line(x-50, y+50, x+50, y-50, outline= "green", width=3)
